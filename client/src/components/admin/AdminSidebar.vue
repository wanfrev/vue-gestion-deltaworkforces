    ...
    >
      <div class="flex h-full flex-col overflow-y-auto px-5 py-6 md:sticky md:top-0 md:px-6">
        ...
        <button
          ...
          @click="onSectionClick('cargar-nomina')"
        >
          <span class="absolute bottom-1.5 left-0 top-1.5 w-1 rounded-r-full transition" :class="seccionActiva === 'cargar-nomina' ? 'bg-delta-blue' : 'bg-transparent group-hover:bg-slate-600/60'"></span>
          <Upload :size="16" :stroke-width="2" :class="seccionActiva === 'cargar-nomina' ? 'text-blue-300' : 'text-slate-400 group-hover:text-slate-100'" />
        ...
        <button
          ...
          :class="seccionActiva === 'gestionar-empleados'
            ? 'border-slate-700 bg-slate-800/85 text-slate-50 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]'
            : 'border-transparent text-slate-300 hover:border-slate-700/85 hover:bg-slate-800/65 hover:text-slate-100'"
          @click="onSectionClick('gestionar-empleados')"
        >
          <span class="absolute bottom-1.5 left-0 top-1.5 w-1 rounded-r-full transition" :class="seccionActiva === 'gestionar-empleados' ? 'bg-delta-blue' : 'bg-transparent group-hover:bg-slate-600/60'"></span>
          <Users :size="16" :stroke-width="2" :class="seccionActiva === 'gestionar-empleados' ? 'text-blue-300' : 'text-slate-400 group-hover:text-slate-100'" />
...
const onSectionClick = (section: AdminSection) => {
